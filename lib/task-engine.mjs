...
export async function attachTaskSession(root, slug, taskID, runId, sessionID) {
  ...
  return withWorkLock(root, slug, async () => {
    ...
    return { taskID, runId: record.runId, sessionID: value }
  })
}
...
export async function completeTask(root, slug, taskID, options = {}) {
  ...
  return withWorkLock(root, slug, async () => {
    ...
    if (evidencePolicy.receiptRequiredForTasks && successfulReceipts.length === 0) {
      throw new Error("strict task completion requires at least one successful verification receipt for the active run")
    }
    record.status = "completed"
    record.completedAt = timestamp
    record.report = path.relative(loaded.paths.root, reportPath).replaceAll("\\", "/")
    record.evidenceStrength = successfulReceipts.length ? "receipt-backed" : "narrative"
    record.runId = null
    record.owner = null
    record.heartbeatAt = null
    record.leaseExpiresAt = null
    ...
    loaded.evidence.entries.push({
      ...
      receiptIDs: successfulReceipts.map((item) => item.id),
    })
    loaded.evidence.schemaVersion = STATE_SCHEMA
...
async function buildContextPack(loaded, taskID) {
  ...
  const contextManifest = await buildContextManifest(loaded.paths.root, task, { budget: 24_000 }).catch(() => null)
