...
import { buildMemorySnapshot, recordVerifiedTaskMemory, retrieveMemories } from "./memory-engine.mjs"
...
async function lockOwner(paths) {
  return readJson(path.join(paths.lock, "owner.json"), null).catch(() => null)
}
...
async function acquireWorkLock(paths) {
  ...
  while (true) {
    ...
    try {
      ...
      const owner = { pid: process.pid, token, at: now(), heartbeatAt: now() }
      await writeFile(
      ...
      const info = await stat(ownerFile).catch(() => stat(paths.lock).catch(() => null))
      if (info && Date.now() - info.mtimeMs > LOCK_STALE_MS) {
        ...
        try {
          ...
          // Another contender refreshed or took over the lock first.
        }
      }
...
async function heartbeatWorkLock(paths, token) {
  const owner = await lockOwner(paths)
  if (!owner || owner.token !== token) return false
  await writeFile(
    path.join(paths.lock, "owner.json"),
    JSON.stringify({ ...owner, heartbeatAt: now() }) + "\n",
    "utf8",
  )
  return true
}

async function releaseWorkLock(paths, token) {
  const owner = await lockOwner(paths)
  if (!owner || owner.token !== token) return false
...
async function withWorkLock(root, slug, fn) {
  ...
  const owner = await acquireWorkLock(paths)
  const heartbeat = setInterval(() => {
    heartbeatWorkLock(paths, owner.token).catch(() => {})
  }, LOCK_HEARTBEAT_MS)
  heartbeat.unref?.()
  try {
    ...
    await releaseWorkLock(paths, owner.token).catch(() => {})
  }
}
...
const FINGERPRINT_SKIP_DIRS = new Set([
  ".git", ".ues-work", ".ues-learning", ".ues-dashboard", ".ues-sandboxes", ".ues-cache", ".ues-traces", ".ues-memory",
...
export function workspaceFingerprint(root) {
  ...
  const commands = [
    ["rev-parse", "HEAD"],
    ["status", "--porcelain=v1", "--untracked-files=all", "--", ".", ":(exclude).ues-work", ":(exclude).ues-learning", ":(exclude).ues-dashboard", ":(exclude).ues-sandboxes", ":(exclude).ues-cache", ":(exclude).ues-traces", ":(exclude).ues-memory"],
    ["diff", "--binary", "--no-ext-diff", "--", ".", ":(exclude).ues-work", ":(exclude).ues-learning", ":(exclude).ues-dashboard", ":(exclude).ues-sandboxes", ":(exclude).ues-cache", ":(exclude).ues-traces", ":(exclude).ues-memory"],
...
export async function finalizeWork(root, slug, evidence) {
  ...
  return withWorkLock(root, slug, async () => {
    ...
    const memory = await recordVerifiedTaskMemory(loaded.paths.root, {
      task: loaded.state.goal || loaded.plan.goal || `Completed durable work: ${slug}`,
      files: [...new Set(loaded.plan.tasks.flatMap((task) => taskFiles(task)))],
      verifier: verification.receipt?.verifier || "ues-integration-verifier",
      verifierOutput: [verification.evidence, verification.report].filter(Boolean).join("\n"),
      integrationOutput: evidenceText,
      sourceTask: `work:${slug}`,
      taskClass: "long-horizon",
      type: "episodic",
      scope: "project",
    }).catch(() => null)

    return { state: loaded.state, evidence: loaded.evidence.entries.at(-1), memory }
  })
}
...
async function buildContextPack(loaded, taskID) {
  ...
  const memoryRetrieval = await retrieveMemories(loaded.paths.root, taskText, {
    files: taskFiles(task),
    limit: 6,
    taskClass: contextPolicy.executionProfile || contextPolicy.profile?.name || contextPolicy.mode || null,
    touch: true,
  }).catch(() => ({ schemaVersion: 1, query: taskText, eligible: 0, results: [] }))
  const memories = memoryRetrieval.results || []
  const memorySnapshot = memoryRetrieval.snapshot || buildMemorySnapshot(memories)
  const fabric = await capabilityFabricStatus(loaded.paths.root).catch(() => null)
  ...
  const promptEnvelope = buildPromptEnvelope({
    ...
    memorySnapshot,
    contextHints: {
      hierarchy: (contextManifest?.hierarchy?.scopes || []).slice(0, 6).map((item) => ({
        path: item.path,
        score: item.score,
        l0: item.l0,
      })),
  ...
  return {
    ...
    promptCache: {
      ...
      cacheableChars: promptEnvelope.cacheableChars,
      cacheableRatio: promptEnvelope.cacheableRatio,
      memoryGeneration: memorySnapshot.generation,
    },
    attempt,
    ...
    memoryRetrieval: {
      ...
      generation: memorySnapshot.generation,
    },
    capabilityFabric: {
