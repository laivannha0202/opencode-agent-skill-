...
import { buildContextManifest } from "./context-intelligence.mjs"
...
async function appendEvent(paths, type, data = {}) {
  await appendFile(
    paths.events,
    JSON.stringify({ schemaVersion: 1, at: now(), type, ...data }) + "\n",
    "utf8",
  )
}

async function readEvents(file, limit = 100) {
...
function nonGitFingerprint(root) {
  const hash = createHash("sha256")
  const skip = new Set([".git", ".ues-work", ".ues-evals", ".ues-learning", "node_modules", "dist", "build", "coverage"])
  let count = 0

  function visit(dir, depth) {
    if (depth > 8 || count >= 2500) return
    let entries = []
    try {
    ...
    for (const entry of entries) {
      if (count >= 2500 || skip.has(entry.name)) continue
      ...
      if (info.size <= 512 * 1024) {
        try { hash.update(readFileSync(full)) } catch {}
      } else {
...
export async function initWork(root, slug, goal, options = {}) {
  const paths = workPaths(root, slug)
  ...
  await writeJson(paths.state, {
    ...
    evidencePolicy: {
      requireReceipts: options.requireReceipts === true,
    },
    integrationReceipts: [],
    tasks: {},
    decisions: [],
  ...
  await appendEvent(paths, "work.initialized", {
    slug,
    requireReceipts: options.requireReceipts === true,
  })
...
export async function importPlan(root, slug, planInput) {
  ...
  return withWorkLock(root, slug, async () => {
    ...
    await appendEvent(loaded.paths, "plan.imported", { planHash: hash, taskCount: plan.tasks.length })
...
export async function approvePlan(root, slug, evidence) {
  ...
  return withWorkLock(root, slug, async () => {
    ...
    await appendEvent(loaded.paths, "plan.approved", { planHash: hash, ready: next })
...
export async function workStatus(root, slug) {
  ...
  return {
    ...
    evidencePolicy: loaded.state.evidencePolicy || { requireReceipts: false },
    leases: Object.fromEntries(
      Object.entries(loaded.state.tasks || {})
...
export async function startTask(root, slug, taskID, options = {}) {
  const result = await withWorkLock(root, slug, async () => {
    ...
    const timestamp = now()
    const leaseMs = Math.max(30_000, Number(options.leaseMs) || DEFAULT_LEASE_MS)
    const previousFailure = record.lastFailure || null
    ...
    record.ownerSession = options.ownerSession ? String(options.ownerSession) : null
    record.receipts = []
    loaded.state.integrationVerification = null
    loaded.state.integrationReceipts = []
    ...
    await writeJson(loaded.paths.state, loaded.state)
    await appendEvent(loaded.paths, "task.started", {
      task: taskID,
      runId: record.runId,
    ...
    return { task, record: { ...record, previousFailure } }
  })

  return { ...result, contextPack: await contextPack(root, slug, taskID) }
}

export async function heartbeatTask(root, slug, taskID, runId, leaseMs = DEFAULT_LEASE_MS) {
  return withWorkLock(root, slug, async () => {
    const loaded = await loadWork(root, slug)
    const record = loaded.state.tasks?.[taskID]
    if (!record || record.status !== "running") throw new Error("task is not running")
    if (!runId || record.runId !== runId) throw new Error("task runId does not match active lease")
    const timestamp = now()
    record.heartbeatAt = timestamp
    record.leaseExpiresAt = new Date(Date.now() + Math.max(30_000, Number(leaseMs) || DEFAULT_LEASE_MS)).toISOString()
    loaded.state.updatedAt = timestamp
    await writeJson(loaded.paths.state, loaded.state)
    return { taskID, runId, heartbeatAt: record.heartbeatAt, leaseExpiresAt: record.leaseExpiresAt }
  })
}

export async function recoverStaleTasks(root, slug, options = {}) {
  const at = Number(options.nowMs) || Date.now()
  return withWorkLock(root, slug, async () => {
    const loaded = await loadWork(root, slug)
    const recovered = []
    for (const [taskID, record] of Object.entries(loaded.state.tasks || {})) {
      if (record.status !== "running") continue
      const explicitExpiry = record.leaseExpiresAt ? Date.parse(record.leaseExpiresAt) : NaN
      const legacyExpiry = record.startedAt ? Date.parse(record.startedAt) + DEFAULT_LEASE_MS : 0
      const expiry = Number.isFinite(explicitExpiry) ? explicitExpiry : legacyExpiry
      if (Number.isFinite(expiry) && expiry > at) continue
      record.status = "failed"
      record.lastFailure = "executor lease expired before completion"
      record.runId = null
      record.heartbeatAt = null
      record.leaseExpiresAt = null
      record.ownerSession = null
    ...
    if (recovered.length) {
      ...
      loaded.state.nextAction = "Recovered stale task lease(s): " + recovered.join(", ") + ". Re-diagnose and retry."
      await writeJson(loaded.paths.state, loaded.state)
      await appendEvent(loaded.paths, "task.stale-recovered", { tasks: recovered })
    }
    return { recovered, state: loaded.state }
  })
}

export async function recordVerificationReceipt(root, slug, taskID, receiptInput) {
  const checked = validateVerificationReceipt(receiptInput)
  if (!checked.valid) throw new Error("invalid verification receipt: " + checked.errors.join("; "))
  return withWorkLock(root, slug, async () => {
    const loaded = await loadWork(root, slug)
    const receipt = checked.receipt
    const currentFingerprint = workspaceFingerprint(loaded.paths.root)
    if (receipt.workspaceFingerprint !== currentFingerprint) {
      throw new Error("verification receipt fingerprint does not match current workspace")
    }

    let record = null
    if (taskID === "__integration__") {
      if (!loaded.plan) throw new Error("PLAN.json is missing")
      const incomplete = loaded.plan.tasks
        .filter((task) => loaded.state.tasks?.[task.id]?.status !== "completed")
        .map((task) => task.id)
      if (incomplete.length) {
        throw new Error("integration verification receipt requires all tasks complete: " + incomplete.join(", "))
      }
      if (loaded.state.blockers?.length) {
        throw new Error("integration verification receipt cannot be recorded while blockers remain")
      }
      receipt.scope = "integration"
      loaded.state.integrationReceipts ??= []
      loaded.state.integrationReceipts.push(receipt)
    } else {
      record = loaded.state.tasks?.[taskID]
      if (!record) throw new Error("unknown task for verification receipt: " + taskID)
      if (record.status !== "running") throw new Error("task must be running before recording verification")
      if (receipt.runId && receipt.runId !== record.runId) throw new Error("verification receipt runId does not match active task")
      receipt.runId = record.runId
      record.receipts ??= []
      record.receipts.push(receipt)
    }

    const targetDir = path.join(loaded.paths.receipts, taskID)
    await mkdir(targetDir, { recursive: true })
    const file = path.join(targetDir, receipt.id + ".json")
    await writeJson(file, receipt)
    loaded.state.updatedAt = now()
    await writeJson(loaded.paths.state, loaded.state)
    await appendEvent(loaded.paths, "verification.receipt", {
      task: taskID,
      receipt: receipt.id,
      passed: receipt.passed,
      command: receipt.command,
    })
    return { receipt, file: path.relative(loaded.paths.root, file).replaceAll("\\", "/") }
  })
}
...
export async function completeTask(root, slug, taskID, options = {}) {
  ...
  return withWorkLock(root, slug, async () => {
    ...
    if (options.runId && options.runId !== record.runId) throw new Error("task runId does not match active lease")

    const currentFingerprint = workspaceFingerprint(loaded.paths.root)
    const validReceipts = (record.receipts || []).filter((receipt) =>
      receipt.passed &&
      receipt.workspaceFingerprint === currentFingerprint &&
      (!receipt.runId || receipt.runId === record.runId),
    )
    if (loaded.state.evidencePolicy?.requireReceipts && validReceipts.length === 0) {
      throw new Error("completion requires at least one passing structured verification receipt for the current workspace")
    }
    ...
    loaded.evidence.entries.push({
      ...
      verificationReceipts: validReceipts,
      evidenceStrength: validReceipts.length ? "structured" : "narrative",
    })
    ...
    await writeJson(loaded.paths.state, loaded.state)
    await appendEvent(loaded.paths, "task.completed", {
      task: taskID,
      receipts: validReceipts.map((item) => item.id),
      ready,
    })
...
export async function failTask(root, slug, taskID, reason, options = {}) {
  ...
  return withWorkLock(root, slug, async () => {
    ...
    if (options.runId && record.runId && options.runId !== record.runId) {
      throw new Error("task runId does not match active lease")
    }
    const timestamp = now()
    record.status = "failed"
    ...
    await writeJson(loaded.paths.state, loaded.state)
    await appendEvent(loaded.paths, "task.failed", { task: taskID, reason: cleanReason, attempts: record.attempts })
...
export async function recordIntegrationVerification(root, slug, verdict, evidence, report = null) {
  ...
  return withWorkLock(root, slug, async () => {
    ...
    const validReceipts = (loaded.state.integrationReceipts || []).filter((receipt) =>
      receipt.passed && receipt.workspaceFingerprint === fingerprint,
    )
    if (loaded.state.evidencePolicy?.requireReceipts && status === "PASS" && validReceipts.length === 0) {
      throw new Error("integration PASS requires at least one passing structured verification receipt for the current workspace")
    }
    ...
    await writeJson(loaded.paths.evidence, loaded.evidence)
    await writeJson(loaded.paths.state, loaded.state)
    await appendEvent(loaded.paths, "integration.verified", {
      verdict: status,
      receipts: validReceipts.map((item) => item.id),
      fingerprint,
    })
...
export async function finalizeWork(root, slug, evidence) {
  ...
  return withWorkLock(root, slug, async () => {
    ...
    await writeJson(loaded.paths.evidence, loaded.evidence)
    await writeJson(loaded.paths.state, loaded.state)
    await appendEvent(loaded.paths, "work.finalized", { fingerprint: currentFingerprint })
...
async function buildContextPack(loaded, taskID) {
  ...
  const contextManifest = await buildContextManifest(loaded.paths.root, task)
...
export async function resumeWork(root, slug) {
  const recovery = await recoverStaleTasks(root, slug)
  const loaded = await loadWork(root, slug)
  return {
    ...
    decisions: loaded.state.decisions || [],
    recovery,
    recentEvents: await readEvents(loaded.paths.events),
  }
}
