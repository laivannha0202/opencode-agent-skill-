...
export class McpHealthTracker {
  ...
  finish(callId, result = {}, at = Date.now()) {
    ...
    const cooldownUntil = transient && consecutiveFailures >= this.failureThreshold
      ...
      : failed
        ...
        : 0
    const reconnectAdvice = failed
      ? mcpReconnectAdvice({ policy: active.policy, error: errorText, attempt: consecutiveFailures })
      : null
    const next = {
  ...
  status(toolName, at = Date.now()) {
    ...
    return {
      ...
      reconnectAdvice: row.reconnectAdvice || null,
