...
export function createToolOutputAccumulator(options = {}) {
  const maxChars = Math.max(1024, Math.min(64 * 1024, Number(options.maxChars || 12_000)))
  ...
  return {
    append(toolCallId, text) {
      ...
      const next = (String(values.get(key) || "") + String(text || "")).slice(-maxChars)
      values.set(key, next)
      return next
    },
    get(toolCallId) {
      return String(values.get(String(toolCallId || "")) || "")
    },
    delete(toolCallId) {
      values.delete(String(toolCallId || ""))
    },
    clear() {
      values.clear()
    },
