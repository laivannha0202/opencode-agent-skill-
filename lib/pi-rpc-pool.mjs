...
class RpcWorker {
  ...
  async abort() {
    if (!this.proc || this.dead) return
    await this.send({ type: "abort" }, { timeoutMs: 15_000 }).catch(() => {
  ...
  run(message, options = {}) {
    const execute = async () => {
      ...
      const idleTimer = setInterval(() => {
        if (Date.now() - lastActivityAt >= idleTimeoutMs) void abortFor("idle-timeout")
      }, 1000)
      idleTimer.unref?.()
...
export class PiRpcWorkerPool {
  ...
  async run(key, spec, message, options = {}) {
    ...
    try {
      return await worker.run(message, options)
    } catch (error) {
