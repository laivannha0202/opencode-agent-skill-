...
async function benchmarkValidationFromArtifact(root, reportPath) {
  ...
  if (!report.confidence || report.confidence.kind !== "ues-paired-benchmark-confidence") {
    throw new Error("learning promotion requires paired benchmark confidence evidence")
  }
  if (report.confidence.promotionEligible !== true) {
