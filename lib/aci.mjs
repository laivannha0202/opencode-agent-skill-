...
export async function aciTextSearch(root, query, options = {}) {
  ...
  const semantic = await querySemanticIndex(root, value, { limit: options.candidateLimit ?? 60 })
  const candidates = semantic.results.map((item) => item.path)
  const matches = []
  ...
  for (const file of candidates) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
      ...
      if (matches.length >= Math.max(1, Math.min(Number(options.limit || 80), 200))) break
    }
    if (matches.length >= Math.max(1, Math.min(Number(options.limit || 80), 200))) break
  }
  return {
