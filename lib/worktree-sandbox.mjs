...
export async function integrateTaskSandbox(root, dir, options = {}) {
  ...
  const applied = git(root, ["apply", "--3way", "--whitespace=nowarn", "-"], { input: patch.stdout })
  if (applied.status !== 0) {
    throw new Error((applied.stderr || applied.stdout || "git apply --3way failed").trim())
  }
...
export async function removeTaskSandbox(root, dir, options = {}) {
  ...
  if (options.deleteBranch && branch) git(root, ["branch", "-D", branch])
