...
function filesForKeys(task, keys) {
...
export function taskWriteFiles(task) {
  return filesForKeys(task, ["create", "modify", "test", "delete"])
}

export function taskReadFiles(task) {
  return filesForKeys(task, ["read"])
}

export function taskFiles(task) {
  return [...new Set([...taskWriteFiles(task), ...taskReadFiles(task)])].sort()
}
...
function overlaps(taskA, taskB) {
  ...
  const allA = new Set([...writeA, ...readA])
  const allB = new Set([...writeB, ...readB])
  if (allA.size === 0 || allB.size === 0) return true

  for (const file of writeA) {
    if (writeB.has(file) || readB.has(file)) return true
  }
  for (const file of writeB) {
    if (readA.has(file)) return true
  }
  return false
}
