...
import { resolveNodeShimEntry } from "./windows-shim.mjs"
...
function runOpenCodeVersion() {
  ...
  const resolved = findWindowsCommand("opencode")
  if (!resolved) return { status: 127, stdout: "", stderr: "opencode not found" }
  ...
  if (/\.(cmd|bat)$/i.test(resolved)) {
    ...
    if (!entry) {
      return {
        status: 126,
        stdout: "",
        stderr: "refusing to execute an unrecognized OpenCode Windows batch shim",
      }
    }
    return spawnSync(process.execPath, [entry, "--version"], { encoding: "utf8" })
  }

  return spawnSync(resolved, ["--version"], { encoding: "utf8" })
}
