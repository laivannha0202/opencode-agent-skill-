...
export function capabilitiesFromHelp(versionOutput, runHelpOutput = "") {
  ...
  return {
    schemaVersion: 1,
    major,
    supportsStandalone: /(^|\s)--standalone(?:\s|,|$)/m.test(help),
    supportsJsonFormat: /(^|\s)--format(?:\s|,|$)/m.test(help) && /json/i.test(help),
    supportsAgentFlag: /(^|\s)--agent(?:\s|,|$)/m.test(help),
    supportsModelFlag: /(^|\s)--model(?:\s|,|$)/m.test(help),
    supportsDirFlag: /(^|\s)--dir(?:\s|,|$)/m.test(help),
  }
}
