...
export function diffTaskSandbox(root, slug, taskID) {
  ...
  if (files.length) {
    const intent = git(dir, ["add", "-N", "--", ...files])
    if (intent.status !== 0) throw new Error((intent.stderr || intent.stdout || "git add -N failed").trim())
  }

  const diff = git(dir, ["diff", "--binary", "HEAD"])
  if (diff.status !== 0) throw new Error((diff.stderr || diff.stdout || "git diff failed").trim())
  return { dir, patch: diff.stdout, changed: Boolean(diff.stdout.trim()), untracked: files }
}
