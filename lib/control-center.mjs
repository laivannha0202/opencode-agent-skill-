...
async function load(){
 ...
 document.getElementById("work").innerHTML=data.workItems.map(w=>`<div class="card"><div class="row"><b>${esc(w.slug)}</b><span class="badge">${esc(w.status)}</span></div><p class="muted">${esc(w.goal)}</p><div class="tasks">${Object.entries(w.tasks).map(([id,t])=>`<div class="task row"><span>${esc(id)}</span><span>${esc(t.status)} · attempts ${esc(t.attempts)}</span></div>`).join("")}</div><p>Evidence: ${w.evidenceCount} · Events: ${w.events.length}</p><pre>${esc(JSON.stringify(w.events.slice(-8),null,2))}</pre></div>`).join("")||'<div class="card muted">No .ues-work items</div>'
 document.getElementById("evals").innerHTML=data.evals.map(e=>`<div class="card"><b>${esc(e.suite)} · ${esc(e.model)}</b><p class="muted">${esc(e.file)}</p><pre>${esc(JSON.stringify(e.summary,null,2))}</pre></div>`).join("")||'<div class="card muted">No eval runs</div>'
 const ps=data.learning?.proposals||[]
 document.getElementById("learning").innerHTML=ps.map(p=>`<div class="card"><div class="row"><b>${esc(p.signature)}</b><span class="badge">${esc(p.confidence)}</span></div><p>${esc(p.recommendation)}</p><small>${p.evidenceCount} evidence samples · proposal only</small></div>`).join("")||'<div class="card muted">No learning proposals</div>'
}
