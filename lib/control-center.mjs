...
function render(next){
 ...
 for(const item of data.work){
  ...
  work.insertAdjacentHTML("beforeend",'<div class="card"><div><span class="pill">'+esc(item.status)+'</span></div><h3>'+esc(item.slug)+'</h3><div class="muted">'+esc(item.goal)+'</div><p>'+esc(item.nextAction)+'</p>'+tasks+'<div class="muted">receipts: '+item.receipts.length+' · blockers: '+item.blockers.length+'</div>'+recover+'</div>');
 }
