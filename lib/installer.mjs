...
function validSkillID(value) {
  return typeof value === "string" && /^ues-[a-z0-9]+(?:-[a-z0-9]+)*$/.test(value)
}

function validManagedMarkdown(value) {
  return typeof value === "string" && /^ues-[a-z0-9]+(?:-[a-z0-9]+)*\.md$/.test(value)
}

async function readManagedState(file, warnings = []) {
  const raw = await readText(file)
  if (!raw) return { skills: [], commands: [], agents: [] }
...
export async function installResources() {
  ...
  const previousState = await readManagedState(stateFile, warnings)
...
export async function removeResources() {
  ...
  const warnings = []
  const state = await readManagedState(stateFile, warnings)
...
export async function getStatus() {
  ...
  if (state.package && state.package !== PACKAGE_NAME) {
    return {
      installed: false,
      configDir,
