...
async function readManagedState(file, warnings = []) {
  ...
  if (!raw) return { kind: "empty", state: { skills: [], commands: [], agents: [] } }
  ...
  try {
    ...
    return { kind: "invalid", state: { skills: [], commands: [], agents: [] } }
  }
}
...
async function scanUntrackedManaged(configDir) {
  const found = { skills: [], commands: [], agents: [] }
