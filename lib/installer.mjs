...
export async function installResources(options = {}) {
  ...
  for (const entry of agentEntries) {
    ...
    const source = adaptAgentForOpenCode(
  ...
  if (openCodeMajor >= 2) {
    const sourcePlugin = path.join(sourceRoot, "global-config", "plugins", "ues-router", "index.js")
    const targetPlugin = path.join(pluginsTarget, ROUTER_PLUGIN_STATE)
    if (existsSync(sourcePlugin)) {
      const source = await readFile(sourcePlugin, "utf8")
      const content = source.includes(MANAGED_MARKER)
        ? source
        : `${source.trimEnd()}\n\n// ${MANAGED_MARKER}\n`
      if (await writeManagedFile(targetPlugin, content, warnings)) {
