...
test("Pi adapter and prompt resources are packaged", () => {
  ...
  assert.match(source, /name:\s*"ues_service"/)
  assert.match(source, /pi\.registerCommand\("ues-run"/)
  assert.match(source, /ues_controller_direct/)
  assert.match(source, /looksLikeLongRunningServiceCommand/)
  ...
  assert.match(source, /auditCompletion/)
  assert.match(source, /mcpExecutionPolicy/)
  ...
  assert.match(source, /function isAbortedRun/)
  assert.match(source, /const abortedResponse/)
  assert.match(source, /scheduled\.aborted === true/)
  assert.match(source, /reason: "aborted"/)
  ...
  const serviceSource = fs.readFileSync(path.join(root, "lib", "service-manager.mjs"), "utf8")
  assert.match(serviceSource, /startService/)
  assert.match(serviceSource, /waitForService/)
  assert.match(serviceSource, /terminateProcessTree/)
  assert.match(serviceSource, /kind:\s*"service-log"/)
  ...
  assert.match(evalSource, /prompt = "\/ues-run " \+ task\.prompt/)
  assert.match(evalSource, /UES_EVAL_DIRECT_TELEMETRY/)
  assert.match(evalSource, /ues_controller_direct/)
