...
const routerTextRuntime = path.join(pluginsRoot, "ues-router", "text-runtime.js")
const routerVerifierRuntime = path.join(pluginsRoot, "ues-router", "verifier-runtime.js")
const routerCommandRuntime = path.join(pluginsRoot, "ues-router", "command-runtime.js")
...
if (!existsSync(routerTextRuntime)) errors.push("missing V13 UTF-aware router text runtime")
if (!existsSync(routerVerifierRuntime)) errors.push("missing V13 verifier verdict runtime")
if (!existsSync(routerCommandRuntime)) errors.push("missing V13 V2 prompt alias runtime")
if (existsSync(routerIndex)) {
  ...
  if (!source.includes("function spawnHidden")) errors.push("v2 router plugin must centralize subprocesses through spawnHidden")
  if (!source.includes("windowsHide: true")) errors.push("v2 router plugin must hide Windows child-process consoles")
  if ((source.match(/\bspawnSync\s*\(/g) || []).length !== 1) errors.push("v2 router plugin must not call spawnSync directly outside spawnHidden")
  ...
  if (!source.includes("expandUesPromptAlias")) errors.push("v13 router plugin must expand UES slash aliases through session.prompt")
  if (!source.includes("command-templates")) errors.push("v13 router plugin must load managed prompt alias templates")
  if (!source.includes('name: "text_read"')) errors.push("v13 router plugin must expose UTF-aware text recovery")
