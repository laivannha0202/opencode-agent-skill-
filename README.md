...
> **V13 beta: 13.0.0-beta.3** — parallel weak-model runtime với same-model worker pool, event-driven DAG, transactional worktree integration và hardened CLI/Windows runtime.
> **Stable latest vẫn là V11: 11.0.0.** V13 được đóng gói dưới prerelease `13.0.0-beta.3`; npm prerelease dùng dist-tag `next`, còn `latest` tiếp tục giữ V11 cho đến khi V13 được promote stable.
...
Mỗi V13 prerelease, bao gồm beta.3, chỉ được publish sau khi full gate hiện tại PASS. Không hard-code số lượng test vào tài liệu này vì suite tiếp tục tăng; nguồn đúng là output của chính các lệnh gate:
